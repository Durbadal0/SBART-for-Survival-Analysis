BART.surv.loop=function(response=Yt,train=X_Cov,test=X_Cov,county=county,censor=cen,
                        num_iter=5000,adj.matrix=Adj.M){
  ...
  for( iter in 2:10){
    ...
    tree[[iter]]=extract(bart_res,"trees",sampleNums=1000)
    cutoff_t[[iter]]=c(0,sort(unique(tree[[iter]]$value[which(tree[[iter]]$var==1)])),50)
    ...
    pred_test=cbind(rep(cutoff_t[[iter]][1],N),X)
    
    
    for(i in 2:length(cutoff_t[[iter]])){
      
      pred_test=rbind(pred_test,cbind(rep(cutoff_t[[iter]][i],N),X))
    }
    
    
  result_bart[[iter]]= all_tree_bart(tree[[iter]],pred_test)
    
  
  W_N=rep.clus %*% W[iter,]
  
  
    cu_hazard=function(s){
      
      
      if(Y[s]==0){return(0)}
      ind_less=which(cutoff_t[[iter]]<=Y[s])
      
      
      
        n_step=length(ind_less)
        
         step=c(c(cutoff_t[[iter]][1:n_step]),Y[s])
    
    B_step=step[-1]
         
    
    C_step=B_step-c(cutoff_t[[iter]][1:n_step])
    
    haz_s=c()

         for(l in 1: length(C_step)){
           haz_s[l]=h(Y[s],alpha = alpha[iter],
                      beta=1)*c(W_N[s])*
             pnorm(result_bart[[iter]][s+l*N])
         }
  ...
  cu_ha[[iter]]=sapply(1:N,cu_hazard)  
  ...
  if(iter %% 100==0){
    ...
    saveRDS(tree,"bart_tree.RDS")
    saveRDS(cutoff_t,"cutoff.RDS")
    saveRDS(cu_ha,"cum_hazard_ij.RDS")
    saveRDS(result_bart,"prediction_ij.RDS")
  }
